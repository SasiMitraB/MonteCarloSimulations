"""
Random Walk Simulation - Pygame Visualization

This module simulates a random walk in 2D space with multiple particles (balls),
including a central "big ball" and smaller particles that interact via elastic
collisions. The simulation uses Pygame for real-time visualization, NumPy for
numerical computations, and Numba for performance acceleration. Particles
bounce off walls and each other, with random velocity updates each step.

Features
- Multiple particles with elastic collisions (conserving momentum and energy).
- A central "big ball" that influences particle paths.
- Real-time Pygame rendering at 60 FPS.
- Random velocity updates simulating Brownian motion.
- Path tracing for the big ball (in draw_path mode).
...
Author / License
- Educational simulation of random walks and collisions; adapt freely.
"""
